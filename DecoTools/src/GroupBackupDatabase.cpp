...
namespace
{
        ...
        std::string& status)
    {
        ...
        status = type == GroupBackupDatabase::RestorePointType::Safety
            ? "Created safety group restore point."
            : type == GroupBackupDatabase::RestorePointType::Manual
                ? "Created manual group restore point."
                : "Created automatic group restore point.";
        return true;
    }
}
    ...
    const std::string& path, int expectedXmlType, bool automaticEnabled)
{
    ...
    for (auto iterator = restorePoints.rbegin(); iterator != restorePoints.rend(); ++iterator)
    {
        ...
        if (!candidateExact && coverage < 0.50) continue;
    ...
    result.message = "Couldn't find valid entry to auto-restore Groups. Please choose a file to restore from.";
    return result;
}
...
bool GroupBackupDatabase::RebuildXml(const std::string& restorePointId,
    const std::string& outputPath, RebuildResult& result, std::string& status)
{
    result = {};
    const RestorePoint* point = FindRestorePoint(restorePointId);
    if (point == nullptr)
    ...
    const std::string root = point->rootOpenTag.empty()
        ? "<Decorations version=\"1\" type=\"" + std::to_string(point->xmlType) + "\">"
        : point->rootOpenTag;
    ...
    if (!result.completeXml)
        status += " This older backup contained grouped decorations only; original map metadata and ungrouped decorations were not available.";
